...
X <- matrix(c(-3, -1, 0, 2, 4, 2, 4, 4, 3, 6,
...
Visually, the new weight vector, $w_{t+1}$, is determined by adding $y_ix_i$ to $w_t$ and then shifting by the offset $w_0$.
...
shift <- shift(W)
label_vector(W[2] + shift["x"], 
             W[3] + shift["y"],
             W[2] + shift["x"] + mis.point.Y * mis.point[2],
             W[3] + shift["y"] + mis.point.Y * mis.point[3], expression(w[t] + y[i]*x[i]), "below")
...
label_vector(shift["x"], 
             shift["y"],
             W[2] + shift["x"] + mis.point.Y * mis.point[2],
             W[3] + shift["y"] + mis.point.Y * mis.point[3], expression(w[t+1]), "below")
```
